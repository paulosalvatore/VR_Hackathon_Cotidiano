...
#region API
		public void DrawGizmos()
		{
			...
			GizmosExtend.DrawBox(m_Preset.Instance.m_CameraOffsetHandler.transform.parent.position, Vector3.one * .05f, Quaternion.LookRotation(m_Preset.Instance.m_CameraOffsetHandler.transform.parent.position.Direction(GetCameraLookAt().position)), m_Preset.m_DebugColor);
			// Camera position after offset
			GizmosExtend.DrawBox(m_Preset.Instance.CameraPivot.transform.position, Vector3.one * .07f, Quaternion.LookRotation(m_Preset.Instance.CameraPivot.transform.position.Direction(GetCameraLookAt().position)), m_Preset.m_DebugColor);
		}
		...
		internal void Init(Preset preset)
		{
			...
			m_ChaseTargetOffsetHandler.Init(preset, this, Color.white.GetAlpha(.4f));
			
			// Yaw
			tmp = AddChild("Yaw", m_ChaseTargetOffsetHandler.transform);
			m_YawHandler = tmp.AddComponent<ArcYaw>();
			m_YawHandler.Init(preset, this, Color.red.GetAlpha(.05f), Color.red.GetAlpha(.5f));
			...
			m_PitchHandler.Init(preset, this, Color.blue.GetAlpha(.05f), Color.blue.GetAlpha(.5f));
			...
			m_CameraOffsetHandler.Init(preset, this, Color.white.GetAlpha(.4f));
