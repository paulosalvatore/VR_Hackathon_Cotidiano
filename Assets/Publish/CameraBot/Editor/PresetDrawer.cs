...
namespace CF.CameraBot
{
	...
	public class PresetDrawer : PropertyDrawer
	{
		...
		private void OnCameraBotGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			cells = cells[1].Adjust(x: 20f, width: -20f).SplitHorizontal(.85f);
			property.isExpanded = EditorGUI.Foldout(cells[0].Adjust(), property.isExpanded, ((!property.isExpanded) ? preset.gameObject.name : ""));
			...
			if (property.isExpanded)
			{
				...
				line = line.GetRectBottom(height: VirtualPositionDrawer.GetStaticHeight(virtualPositionProp)).Adjust(y: lineS);
				EditorGUI.PropertyField(line, virtualPositionProp, true);

				SerializedProperty methodProp = presetObj.FindProperty("m_Method");
				line = line.GetRectBottom(height: MethodDrawer.GetStaticHeight(methodProp)).Adjust(y: lineS);
