...
public class DataVisualizer : MonoBehaviour
{
	...
	public float ValueScaleMultiplier;
	private GameObject[] seriesObjects;
	private int currentSeries = 0;

	public void CreateMeshes(List<float> allSeries)
	{
		...
		for (int i = 0; i < allSeries.Count - 2; i += 3)
		{
			GameObject objeto = Instantiate(new GameObject());
			objeto.transform.parent = Earth.transform;

			float lat = allSeries[i];
			float lng = allSeries[i + 1];
			float value = allSeries[i + 2];
			...
			{
				CreateObject(meshVertices, meshIndices, meshColors, objeto);
				meshVertices.Clear();
				meshIndices.Clear();
				meshColors.Clear();
			}
